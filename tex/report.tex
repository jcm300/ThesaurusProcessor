...
\begin{itemize}
    \item /\%dom/: guarda-se o dominio indicado numa variável até aparecer uma nova linha com o mesmo padrão altura em que o dominio é atualizado
    \item /\%inv/:': guarda num índice \texttt{inv} a relação e a sua respetiva inversa
    \item /\%THE/: as relações são armazenadas num índice \texttt{relation} e as classes (quando aplicável/existentes) num índice \texttt{class}
    \item \$1 !~/\^\%/ \&\& \$0 !~ /\^\#/ \&\& \$0 != '''' (\textbf{i.e.}tabelas de relações): os triplos são armazenados no índice \texttt{triples}, colocando inicialmente o termo1 relacionado com o dominio(\textbf{dom}) bem como a respetiva inversa(\textbf{voc}). De seguida, caso exista uma classe associada ao '\%THE' esta é adicionada, bem como os triplos correspondentes ao \textbf{instance of} e \textbf{tem como instancia}. O resto dos campos (termos da lado direito) são de seguida percorridos, separando-os por $|$ usando o \texttt{split} e relacionando-os com o domínio atual.
\end{itemize}
