...
\section{Exe 2}
O objetivo deste exercício é mostrar os triplos expandidos(um por cada linha) correspondentes. Como tal, há a necessidade de percorrer os ficheiros por completo. Consideramos como Field Separator ':'. Consoante o início da linha é processado de maneira diferente como indicado de seguida:
\begin{itemize}
    \item linha começa por '\%dom': guarda-se o dominio atual numa variável até aparecer uma nova linha com '\%dom' onde o dominio é atualizado
    \item linha começa por '\%inv :': guarda num indice inv a relação e a sua respetiva inversa
    \item linha começa com '\%THE': é percorrida a linha, guardando as relações e se tiverem as repetivas classes, sendo as relações guardadas num indice relation e as classes num indice class
    \item restantes linhas (\$1 !~/\^\%/ \&\& \$0 !~ /\^\#/ \&\& \$0 != ''''), ou seja, as pertencentes às tabelas '\%THE': é guardado no indice triples os triplos, colocando inicialmente o termo1 relacionado com o dominio(dom) bem como a inversa(voc). Depois verificamos se existe uma classe associada ao '\%THE' e caso exista adicionamos também os triplos correspondentes ao instance of e tem como instancia. Após esta fase, percorremos o resto dos campos(termos da lado direito) separando-os por $|$ usando o textit{split}. De seguida percorremos os elementos já separados relacionando-os com o dominio atual, 
\end{itemize}
